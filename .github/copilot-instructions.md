...
This repository is the `mihome-cloud` adapter for ioBroker. It connects to Xiaomi Mi Home Cloud and creates states for many device classes (air purifiers, vacuums, fans, heaters, sensors, scenes) based on MIoT specs and model-specific mappings.

Adapter-specific behavior to keep in mind:

- Authentication uses Xiaomi QR/URL login flow (no static username/password in adapter config)
- Session/cookie persistence is implemented via adapter state (`auth.session`) and cookie jar handling
- Main runtime is polling-based (`info.connection`, periodic updates, retry after network issues)
- Device/state generation is dynamic and model-dependent (`general`, `status`, `remote`, `custom`, `remotePlugins`, `scenes`)
- Project requires Node.js 20+ and uses ioBroker admin JSON-Config and i18n translation files
...
2. ✅ Use `npx eslint . --fix` (or a repository `lint:fix` script) for auto-fixable issues
...
6. ✅ Keep CI and local scripts aligned. If CI enforces zero warnings, use the same behavior in local checks.

#### Common Issues
- **Unused variables**: Remove or prefix with underscore (`_variable`)
- **Missing semicolons**: Run `npx eslint . --fix`
...
- Use Mocha/Chai for JavaScript unit tests in this repository
...
describe('AdapterName', () => {
  ...
  it('should handle a basic condition', () => {
    const result = true;
...
For `mihome-cloud`, authentication is based on Xiaomi QR/URL login and persisted session cookies, not static credentials.

#### Recommended Pattern for This Adapter

- Prefer deterministic tests for parser/state mapping logic with mocked API responses
- Keep network-dependent login scenarios out of required CI paths
- If an end-to-end login test is added, run it manually or as optional workflow with clear timeout/error output
- Verify robust behavior for session expiration (401), reconnect attempts, and `info.connection` state transitions
...
- **Preferred in general:** Use native `fetch` API (Node.js 20+ required)
- **Repository exception:** Keep `axios` where cookie-jar/redirect/session handling is already implemented and stable
...
api-smoke-tests-optional:
  ...
  steps:
    ...
    - name: Run optional smoke tests
      run: npm run test:integration
```

#### Testing Best Practices
- Run authentication/network smoke tests separately from required main test suite
- Don't make credential tests required for deployment
- Provide clear failure messages for API issues
- Use appropriate timeouts for external calls (120+ seconds)

In this repository, keep required checks aligned with existing scripts from `package.json` (`test`, `test:js`, `test:package`, optional `test:integration`).

---

### Adapter-Specific Patterns (`mihome-cloud`)

- Preserve the QR login user guidance logs; avoid replacing them with opaque errors
- Keep session restoration logic (`loadCookies`, `resumeSession`, 401 recovery) side-effect safe and idempotent
- Maintain interval bounds to prevent invalid polling values and timer overflow
- When adding new device models, update mapping logic in `lib/configDes.js` and ensure stable object IDs/state names
- Prefer defensive parsing for Xiaomi cloud responses (`&&&START&&&`, partial payloads, plugin/API differences)
- Ensure new writable states in `remote`/`remotePlugins` are validated and reset properly after command execution
