...
This adapter (`ioBroker.mihome-cloud`) connects ioBroker to the **Xiaomi Mi Home Cloud API**, enabling control and monitoring of all Mi Home-compatible smart home devices registered in a user's Mi Home account.

### Adapter-Specific Context
- **Adapter Name**: mihome-cloud
- **Primary Function**: Cloud integration for Xiaomi / Mi Home ecosystem — exposes device states, enables control commands, and executes Mi Home scenes via the Xiaomi Cloud REST API.
- **Key Dependencies**: `axios` (HTTP requests to Xiaomi Cloud), `tough-cookie` + `http-cookie-agent` (session/cookie management), `adm-zip` (ZIP extraction for device plugin data), `json2iob` (automatic ioBroker state tree generation from JSON payloads)
- **Authentication**: URL-based OAuth/cookie login flow — no username/password stored in adapter config. The adapter generates a login URL, the user authenticates in the browser, and the resulting session cookie is persisted internally.
- **Supported Regions**: Germany (`de`), China (`cn`), Russia (`ru`), Taiwan (`tw`), Singapore (`sg`), USA (`us`) — each maps to a different Xiaomi Cloud API endpoint.
- **Supported Devices**: Air purifiers (series 2/3/4), fans, heaters, robot vacuums, environment sensors (temperature, humidity), and any other Mi Home-registered device.
- **Configuration Requirements**: Region selection, update polling interval (min 1 minute), optional flag to block additional runtime login attempts.
- **Connection Type**: Cloud (polling via REST API), `dataSource: poll`.

### Key Patterns and Workflows
- **Login Flow**: On start, if no valid session exists, the adapter generates a Xiaomi login URL and exposes it as a warning log message and as state `mihome-cloud.0.auth.loginUrl`. After user authentication the session cookie is stored in `auth.session`.
- **Session Expiry Handling**: On 401 responses the adapter clears the stored session and sets `auth.status = reauth_required`, triggering a re-authentication flow.
- **Device Discovery**: After login, the adapter fetches the device list from the Xiaomi Cloud, creates an ioBroker object tree per device (using `json2iob`), then starts polling device properties at the configured interval.
- **Scene Execution**: Mi Home scenes can be triggered by writing to the corresponding scene state.
- **Error Recovery**: Network failures are handled with retry logic; adapter transitions to `info.connection = false` on persistent failures.
...
#### Mihome-Cloud Specific Integration Test Notes

- The mihome-cloud adapter uses a **browser-based OAuth login**, so full end-to-end integration tests require a valid Mi Home account session.
- For CI/CD, tests should mock the Xiaomi Cloud API responses or use pre-recorded fixture data.
- Example fixture file: `test/fixtures/deviceList.json` — a sample response from the Xiaomi Cloud device list endpoint.
- Integration tests that require real credentials should be placed in `test/integration-demo.js` and run separately via `npm run test:integration-demo`.
- Test that the adapter correctly handles:
  - Missing/expired session (should expose `auth.loginUrl` state)
  - 401 responses (should set `auth.status = reauth_required`)
  - Invalid region configuration
  - Empty device list (should not crash)
  - Network timeout / unreachable cloud endpoint (should set `info.connection = false`)
...
> **Note for this adapter:** The mihome-cloud adapter currently uses `axios` together with `tough-cookie` and `http-cookie-agent` for cookie-jar-aware HTTP requests to the Xiaomi Cloud API. This is an existing dependency — do not migrate away from `axios` without also replacing the cookie-management stack. For all new code not involving the Xiaomi Cloud API session, prefer the native `fetch` API.
...
#### Mihome-Cloud Specific Error Handling

- **401 Unauthorized**: Clear the persisted session (`auth.session`), set `auth.status = reauth_required`, log a warning with the new login URL. Do NOT crash the adapter — wait for re-authentication.
- **403 / Rate limiting**: Back off exponentially; log a warning. The Xiaomi Cloud API has per-account rate limits.
- **Network timeouts**: Set `info.connection = false`, schedule a reconnect attempt after the configured interval.
- **Invalid region**: Log an error at startup; the adapter should not start polling if the configured region is unknown.
- **Empty or malformed device list**: Log a warning; do not create broken state objects. Retry on the next polling cycle.
- **Session persistence errors**: If the session cannot be saved/loaded (filesystem error), log an error but continue operation with an in-memory session.
...
### Mihome-Cloud Adapter-Specific Patterns

#### Xiaomi Cloud API Authentication Flow
- The adapter does NOT store username/password; authentication is fully handled by the Xiaomi OAuth web flow.
- After a successful browser login, the session cookies are captured and persisted in the `auth.session` state.
- When implementing new login-related features, always test the full flow: no session → URL generation → browser auth → session capture → polling start.

#### Region-Based API Endpoints
- Each region maps to a specific Xiaomi Cloud base URL. When adding new regions or modifying the endpoint logic, update both the `jsonConfig.json` dropdown and the endpoint mapping in the adapter code.
- Test with at least the `de` and `cn` regions, as they have different API behaviors.

#### Device State Tree (json2iob)
- Device properties are automatically mapped to ioBroker states using `json2iob`. Avoid manually creating state objects for device properties — let `json2iob` handle the mapping.
- When adding support for new device models, focus on correctly parsing the Xiaomi API response and passing it to `json2iob`.

#### Polling Interval Validation
- The adapter enforces a minimum polling interval of 1 minute. Always validate the `interval` config value at startup and clamp it to the allowed range before scheduling the polling timer.
- The config field uses minutes; convert to milliseconds (`interval * 60 * 1000`) when calling `setInterval`.
