...
class MihomeCloud extends utils.Adapter {
    /**
     * @param {Partial<utils.AdapterOptions>} [options] Adapter constructor options
     */
    constructor(options) {
        super({
            ...options,
    ...
    async onReady() {
        ...
        if (cookiesLoaded) {
            ...
            if (this.session && this.session.ssecurity) {
                ...
                const isCorrupted =
                    this.session.ssecurity.length < 20 || !/^[A-Za-z0-9+/=]+$/.test(this.session.ssecurity);
    ...
    async ensureAuthStates() {
        ...
        const deprecatedAuthStates = ['auth.nextLoginAttempt', 'auth.reauthAttempts'];
        for (const stateId of deprecatedAuthStates) {
            try {
                ...
                this.log.debug(`Failed to remove deprecated auth state ${stateId}: ${error.message}`);
            }
        }
    }
    ...
    scheduleReauthAttempt(reason, forceImmediate = false) {
        ...
        if (this.disableRuntimeLoginAttempts && this.updateInterval) {
            ...
            this.log.warn(`Automatic runtime login attempts are disabled, skipping scheduled retry (${reason})`);
            return;
        }
        ...
        if (this.reauthTimer && this.reauthScheduledAt && this.reauthScheduledAt <= dueAt) {
            return;
        }
        ...
        this.log.warn(`Next QR login attempt scheduled in ${Math.ceil(delay / 1000)}s (${reason})`);
    ...
    async performReauth(reason, forceImmediate = false) {
        ...
        if (!forceImmediate && this.disableRuntimeLoginAttempts && this.updateInterval) {
            await this.updateAuthRuntime('reauth_required');
            return false;
        }
            ...
            if (response.status === 200 && response.data) {
                ...
                if (data.qr) {
                    ...
                    this.session.timeout =
                        Number.isFinite(timeoutSeconds) && timeoutSeconds > 0 ? timeoutSeconds : this.QR_CODE_TIMEOUT;
                    ...
                    .then(async res => {
                        try {
                            ...
                            if (error.message && error.message.includes('zip format')) {
                                this.log.info(`Could not extract plugin zip for ${plugin.model} - invalid format`);
                            } else {
                                this.log.debug(`Failed to extract plugin for ${plugin.model}: ${error.message}`);
                            }
                            return;
                        }
                    })
    ...
    async createStatesFromConfigDes(device) {
        for (const config of configDes) {
            ...
            const isVacuumDevice =
                device.model &&
                (device.model.startsWith('roborock.vacuum.') || device.model.startsWith('rockrobo.vacuum.'));
            ...
            try {
                ...
                if (service.properties) {
                    for (const property of service.properties) {
                        ...
                        this.specPropsToIdDict[device.did][`${property_data.siid}-${property_data.piid}`] =
     ...
     *
     * @param {string} element MIoT property format string (e.g. 'bool', 'int32', 'uint8')
     * @param {boolean} write Whether the property is writable
     * @param {number[] | undefined} valueRange Optional [min, max, step] range for numeric properties
        ...
        for (const device of this.deviceArray) {
            ...
            for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
                ...
                try {
                    ...
                    this.log.error(`Failed to poll custom properties for ${device.name}: ${error.message}`);
                    return;
                }
    ...
    async updateVacuumStatus() {
        ...
        for (const did of this.vacuumStatusDevices) {
            ...
            try {
                ...
                this.log.error(`Failed to poll vacuum status for ${device.name}: ${error.message}`);
                return;
            }
            const { nonce, data_rc, rc4_hash_rc, signature, rc4 } = requestBody;
            const cookieHeader = await this.buildCookieHeader();
                ...
                .then(async res => {
                    ...
                    if (res.data.result && Array.isArray(res.data.result) && res.data.result.length > 0) {
                        ...
                        for (const [field, value] of Object.entries(status)) {
                            ...
                            const needsUpdate =
                                !stateObj ||
                                (propDef && (stateObj.common.unit !== propDef.prop_unit || !stateObj.native?.prop_key));
                ...
                await this.requestClient({
                    ...
                    .then(async res => {
                        try {
                            ...
                            this.log.debug(`Error parsing spec response for ${device.name}: ${error.message}`);
                            return;
                        }
                        if (res.data.code !== 0) {
                        ...
                        if (!this.specPropsToIdDict[device.did]) {
                            this.log.debug(`No spec properties dictionary found for ${device.did}`);
                            return;
                        }
     ...
     *
     * @param {object} error - The error from axios
    ...
    handleAuthError(error, context) {
        if (error.response && (error.response.status === 401 || error.response.status === 400)) {
            ...
            if (this.disableRuntimeLoginAttempts && runtimeActive) {
                this.log.warn('Automatic runtime login attempts are disabled by configuration.');
                return true;
            }
    ...
    async refreshToken() {
        this.log.warn('Session appears to be invalid - scheduling re-authentication');
        this.setState('info.connection', false, true);
    ...
    async loadCookies() {
        try {
            ...
            const accountChanged =
                cookieData.userId && this.session.userId && cookieData.userId !== this.session.userId;
     ...
     *
     * @param {() => void} callback Must be called after cleanup is complete
     */
     ...
     *
     * @param {string} id The full state ID that changed
     * @param {ioBroker.State | null | undefined} state The new state value, or null/undefined if deleted
     */
                ...
                try {
                    ...
                    await this.requestClient({
                        ...
                        .then(async res => {
                            ...
                            if (res.data.code !== 0) {
                                this.log.error(`Error setting state ${id}: ${res.data.message || 'unknown status'}`);
                                this.log.error(JSON.stringify(res.data));
                                return;
                            }
                            ...
                            if (result.out) {
                                if (!this.specActionsToIdDict[result.did]) {
                                    this.log.debug(`No spec actions dictionary found for ${result.did}`);
                                    return;
                                }
                                ...
                                if (stateObject && stateObject.native.out) {
                                    ...
                                    for (const outItem of out) {
                                        ...
                                        if (!this.specPropsToIdDict[result.did]) {
                                            this.log.debug(`No spec properties dictionary found for ${result.did}`);
                                            continue;
                                        }
